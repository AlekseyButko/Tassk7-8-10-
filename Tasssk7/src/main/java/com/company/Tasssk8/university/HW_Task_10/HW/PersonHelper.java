...
/*    public static void createFileWithOutputResultURL(List<String> urls) throws IOException {
        File outFile = new File("resources"+File.separator+"Output_Result_From_URL.csv");
        if(!outFile.exists()) outFile.createNewFile();
        ...
        for (var url : urls) {
            fw.write("\n" + url.replace("zp-", "") + "\n");
            List<Person> persons = getPersonFromURL(url);
